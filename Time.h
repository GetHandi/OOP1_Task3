...
class Time
{
	///объекты класса H, M, S - часы, минуты, секунды;
	int H, M, S;
...
public:
	...
	/// Геттер для часов (не изменяет объекты);
	/// H - значение часов;
	int GetH() const; //сconst означ


	/// Геттер для мину (не изменяет объекты)т;
	/// M - значение минут;
	int GetM() const;


	/// Геттер для секунд (не изменяет объекты);
	/// S - значение секунд;
	int GetS() const;
	...
	/// path - расположение текстового файла;
	void fromFile(const string& path); // константное значение 
